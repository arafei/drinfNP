...
#rm(list=ls())
setwd("/home/arafei/MyJobs/Thesis/Chapter V/Simulation/BS/Own4/Binomial/Design 2stage")
...
V1=4; V2=1; rho=0.5; sigma=0.8; lambda=0.3;
nSim <- 1008
N <- 50000
N_na <- 50
N_a <- N/N_na
...
pop$Y1 <- getY(theta=c(-7, 1, 1, -0.1, 0.2), X=pop[, c("X1", "D", "strat", "XD")], ICC=0.2, a=N_a, na=N_na, sigma=2, family="binomial")
pop$Y2 <- getY(theta=c(-7, 1, 1, -0.1, 0.2), X=pop[, c("X2", "D", "strat", "XD")], ICC=0.2, a=N_a, na=N_na, sigma=2, family="binomial")
pop$Y3 <- getY(theta=c(-7, 1, 1, -0.1, 0.2), X=pop[, c("X3", "D", "strat", "XD")], ICC=0.2, a=N_a, na=N_na, sigma=2, family="binomial")
pop$Y4 <- getY(theta=c(-7, 1, 1, -0.1, 0.2), X=pop[, c("X4", "D", "strat", "XD")], ICC=0.2, a=N_a, na=N_na, sigma=2, family="binomial")
...
res <- foreach(i=1:nSim, .combine=combine, .multicombine = TRUE, .packages = c("MASS", "survey", "rstan", "polyapost", "mgcv")) %dopar% {
	...
	#start_time <- Sys.time()
	 out10 <- tryCatch(maGPPP(psmp=smp0[, c("X", "X0", "XD", "Z0", "D", "D2", "strat", "psu", "wght", "Y")], nsmp=smp1[, c("X", "X0", "XD", "Z0", "D", "D2", "strat", "psu", "wght", "Y")], N=N, method=c("GPPP", "PSPP", "LWP", "AIPW"), spec=c(T, T), family="binomial", Bt1=nBt1, Bt2=nBt2), error = function(e){outx})
	 out11 <- tryCatch(maGPPP(psmp=smp0[, c("X", "X0", "XD", "Z0", "D", "D2", "strat", "psu", "wght", "Y")], nsmp=smp1[, c("X", "X0", "XD", "Z0", "D", "D2", "strat", "psu", "wght", "Y")], N=N, method=c("GPPP", "PSPP", "LWP", "AIPW"), spec=c(F, T), family="binomial", Bt1=nBt1, Bt2=nBt2), error = function(e){outx})
	 out12 <- tryCatch(maGPPP(psmp=smp0[, c("X", "X0", "XD", "Z0", "D", "D2", "strat", "psu", "wght", "Y")], nsmp=smp1[, c("X", "X0", "XD", "Z0", "D", "D2", "strat", "psu", "wght", "Y")], N=N, method=c("GPPP", "PSPP", "LWP", "AIPW"), spec=c(T, F), family="binomial", Bt1=nBt1, Bt2=nBt2), error = function(e){outx})
	 out13 <- tryCatch(maGPPP(psmp=smp0[, c("X", "X0", "XD", "Z0", "D", "D2", "strat", "psu", "wght", "Y")], nsmp=smp1[, c("X", "X0", "XD", "Z0", "D", "D2", "strat", "psu", "wght", "Y")], N=N, method=c("GPPP", "PSPP", "LWP", "AIPW"), spec=c(F, F), family="binomial", Bt1=nBt1, Bt2=nBt2), error = function(e){outx})
	#end_time <- Sys.time()
...
write.csv(res1, paste0("res1_TT_glm_BS_", n0, "_", n1, "_", rho, "_", lambda, "_3.csv"), col.names=T, sep=T)
...
write.csv(tb, paste0("tb_TT_glm_BS_", n0, "_", n1, "_", rho, "_", lambda, "_3.csv"), col.names=T, sep=",")
